function generate_Lψ(ϕ, d::Domain; T = Float64)
    ...
    function Lψ!(y::AbstractVector, x::AbstractVector)
        Δx² = d.Δx^2
        Δy² = d.Δy^2
    ...
    return LinearMap{T}(Lψ!, prod(size(d)); ismutating = true)
end

function generate_Lϕ(ψ, d::Domain; T = Float64)

    function Lϕ!(y::AbstractVector, x::AbstractVector)
        Δx² = d.Δx^2
        Δy² = d.Δy^2
        Δz² = d.Δz^2
        c = CartesianIndices(d)
    ...
    return LinearMap{T}(Lϕ!, prod(size(d)); ismutating = true)
end
